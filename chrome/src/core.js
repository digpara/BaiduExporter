            ...
            fillData: function(file_list) {
                ...
                if (file_list.length > 0) {
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        var cmd_line = "sleep 1 && aria2c --console-log-level=error  -c -t1 -m0 --connect-timeout=1 --auto-file-renaming=false -s64 -k50K -x64 -j64 --enable-rpc=false -o " + filename + CORE.getHeader("aria2c_line") + " " + JSON.stringify(file_list[i].link);
                        aria2c_txt_item = [
                            file_list[i].link,
